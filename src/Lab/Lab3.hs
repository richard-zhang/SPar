...
data Pf i next where
    ...
    Select :: (CC a, CC b) => Sing (n :: Nat) -> Core (Either a b) -> (Core a -> next) -> (Core b -> next) ->  Pf ('Free ('Se n ('Pure ()) ('Pure ()))) next

data TPf next where
    S :: Nat -> a -> next -> TPf next
    R :: Nat -> a -> next -> TPf next
    B :: Nat -> next -> next -> TPf next
    Se :: Nat -> next -> next -> TPf next
...
type family (>*>) (a :: TypeP c) (b :: TypeP c) :: TypeP c where
    ...
    'Free ('B r n1 n2) >*> b = 'Free ('B r (n1 >*> b) (n2 >*> b))
    'Free ('Se r n1 n2) >*> b = 'Free ('Se r (n1 >*> b) (n2 >*> b))
    'Pure _ >*> b = b
...
instance Functor (Pf i) where
    ...
    fmap f (Branch r n1 n2) = Branch r (f n1) (f n2)
    fmap f (Select r v cont1 cont2) = Select r v (f . cont1) (f . cont2)
...
data STypeP (i :: TypeP k) where
    ...
    STb :: STypeP a -> STypeP b -> STypeP ('Free ('B c a b))
    STse :: STypeP a -> STypeP b -> STypeP ('Free ('Se c a b))
...
instance (WitnessTypeP a, WitnessTypeP b) => (WitnessTypeP ('Free ('B c a b))) where
    witness = STb witness witness

instance (WitnessTypeP a, WitnessTypeP b) => (WitnessTypeP ('Free ('Se c a b))) where
    witness = STse witness witness
...
appRightId (STb a b) = case (appRightId a, appRightId b) of
    (Refl, Refl) -> Refl
appRightId (STse a b) = case (appRightId a, appRightId b) of
    (Refl, Refl) -> Refl
...
appAssoc (STr a) y z = case appAssoc a y z of
    Refl -> Refl
appAssoc (STb a b) y z = case (appAssoc a y z, appAssoc b y z) of
    (Refl, Refl) -> Refl
appAssoc (STse a b) y z = case (appAssoc a y z, appAssoc b y z) of
    (Refl, Refl) -> Refl
...
-- select :: (CC a, CC b, CC c) => Sing n -> Core (Either a b) -> (Core a -> P left c) -> (Core b -> P right c) -> P ('Free ('Se n left right )) c
-- select role var cont1 cont2 = Wrap $ Select role var cont1 cont2 
...
type family Project (a :: TypeP c) (r :: Nat) :: TypeP c where
    ...
    Project ('Free ('B r0 next1 next2)) r0 = 'Free ('B r0 (Project next1 r0) (Project next2 r0))
    Project ('Free ('Se r0 next1 next2)) r0 = 'Free ('Se r0 (Project next1 r0) (Project next2 r0))
    Project ('Free ('S r0 v next)) r1 = Project next r1
    Project ('Free ('R r0 v next)) r1 = Project next r1
    Project ('Free ('B r0 next1 next2)) r1 = ProjectHelper (Project next1 r1) (Project next2 r1)
    Project ('Free ('Se r0 next1 next2)) r1 = ProjectHelper (Project next1 r1) (Project next2 r1)
...
type family Dual (a :: TypeP c) (r :: Nat) :: TypeP c where
    ...
    Dual ('Free ('B r0 next1 next2)) r1 = 'Free ('Se r1 (Dual next1 r1) (Dual next2 r1))
    Dual ('Free ('Se r0 next1 next2)) r1 = 'Free ('B r1 (Dual next1 r1) (Dual next2 r1))
