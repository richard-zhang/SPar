...
appAssoc (SSucc xs) ys zs = case appAssoc xs ys zs of
    Refl -> Refl

appRightId :: SList xs -> xs :~: (xs ++ '[])
appRightId SNil       = Refl
appRightId (SSucc xs) = case appRightId xs of
    Refl -> Refl
...
liftF = case appRightId (witness :: SList i) of
    Refl -> Free . fmap1 Return
